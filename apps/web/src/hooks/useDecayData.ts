...
function buildPreviewDecayData(items: HomeProtocolPeptide[]): DecayData {
  const eligibleItems = items.filter((i) => i.peptide.half_life_hours !== null)
  const nowMs = Date.now()
  const results: DecayPeptideResult[] = []
  ...
  for (const item of eligibleItems) {
    ...
    const peakRemainingMcg =
      item.todays_logs.length > 0
        ? Math.max(...item.todays_logs.map((l) => l.dose_mcg))
        : item.dose_mcg
...
export function useDecayData(items: HomeProtocolPeptide[]) {
  ...
  return useQuery({
    ...
    queryFn: () =>
      PREVIEW_MODE
        ? Promise.resolve(buildPreviewDecayData(items))
        : fetchDecayData(items, userId),
    enabled: (PREVIEW_MODE || !!userId) && items.length > 0,
    staleTime: 5 * 60 * 1000,
  })
}
